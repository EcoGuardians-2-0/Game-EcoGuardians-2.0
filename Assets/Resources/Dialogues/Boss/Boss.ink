-> intro

=== intro ===
    Hola mi nombre es Jose y soy una guia en <b><color=\#00FF00>AWAQ</color></b>. #speaker:Jose#animation:5
* -> question

=== question ===
    Dime, ¿En qué te puedo ayudar? #animation:3
    + [Cuentame un chiste]
        -> JOKE
    + [¿Qué haces en el parque?]
        Ayudo a<color=\#FFFF00> resolver inquietudes</color>. 
        -> question
    + [Nada estoy bien]
        Okay estaré aquí por si me necesitas.
        -> DONE

=== JOKE ===
    -¿Qué le dijo<color=\#5D3FD3> una mora a otra </color>? #animation:1
    -¡Tu me ena-moras!
    + [Fue muy gracioso]
        -> question
    + [No me gusto el chiste]
        Okay tal vez no fue gracioso pero... #animation:3
        -> question

* -> END
    


