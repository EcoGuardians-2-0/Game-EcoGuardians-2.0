...
public class lb_Bird : MonoBehaviour
{
	...
	void PlaySong()
	{
		if (!dead)
		{
			if (Random.value < .5)
			{
				// Get the audio source song one and apply the sfx audio mixer to control the volume
				GetComponent<AudioSource>().outputAudioMixerGroup = sfxAudioMixer;
